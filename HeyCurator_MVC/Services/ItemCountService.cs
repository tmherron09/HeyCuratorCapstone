...
namespace HeyCurator_MVC.Services
{
    public class ItemCountService
    {
        ...
        {
            var itemInstanceIds = _context.ItemInstances.Where(ii => ii.ItemId == item.ItemId).Select(ii=> ii.Id).ToList();

            if(itemInstanceIds == null || itemInstanceIds.Count() == 0)
            {
                return new List<Storage>();
            }

            List<Storage> storages = _context.StorageItemInstances.Where(sii => itemInstanceIds.Contains(sii.ItemInstanceId)).Include(sii => sii.Storage).Select(sii => sii.Storage).ToList();
